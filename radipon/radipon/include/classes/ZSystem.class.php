...
class ZSystem
{
	...
	static private function BuildINI($ini) {
		...
		if(!$ini['system']['sitename']) {
			$ini['system']['sitename'] = INCLUDE_CLASSES_ZSYSTEM_CLASS_SITENAME;
		}
		if(!$ini['system']['sitetitle']) {
			$ini['system']['sitetitle'] = INCLUDE_CLASSES_ZSYSTEM_CLASS_SITETITLE;
		}
		if(!$ini['system']['abbreviation']) {
			$ini['system']['abbreviation'] = INCLUDE_CLASSES_ZSYSTEM_CLASS_ABBREVIATION;
		}
		if(!$ini['system']['couponname']) {
			$ini['system']['couponname'] = INCLUDE_CLASSES_ZSYSTEM_CLASS_COUPONNAME;
		}
		if(!$ini['system']['currency']) {
			$ini['system']['currency'] = INCLUDE_CLASSES_ZSYSTEM_CLASS_CURRENCY;
		}
		return $ini;
	}
	...
	static public function GetTemplateList_m() {
		$root = WWW_ROOT . '/m/include/template';
		$dirs = scandir($root);
		$templatelist = array( 'default' => 'default',);
		foreach($dirs AS $one) {
			if ( strpos($one,'.') === 0 ) continue;
	...
	static public function GetThemeList_m() {
		$root = WWW_ROOT . '/m/static/theme';
		$dirs = scandir($root);
		$themelist = array( 'default'=> 'default',);
		foreach($dirs AS $one) {
			if ( strpos($one,'.') === 0 ) continue;
