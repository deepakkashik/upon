...
if (is_post()) {
	...
	Session::Set('once_notice', FORUM_TOPIC_PUBLISH_REPLY_SUCCEED);
	Utility::Redirect( WEB_ROOT . "/forum/topic.php?id={$id}");
}

$pagetitle = sprintf(FORUM_TOPIC_ESPACEFORUM_TITLE,$topic['title']);
$count = $topic['reply_number'];
list($pagesize, $offset, $pagestring) = pagestring_m($count, 10);
