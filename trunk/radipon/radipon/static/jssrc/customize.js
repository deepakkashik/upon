...
window.x_init_hook_click = function() {
	...
	jQuery('#mobile-zone-div input').click(function(){
		var v = jQuery(this).attr('value');
		if ( v == '1' ) {
			jQuery('#is-shown-on').css('display', 'block');
			jQuery('#is-shown-off').css('display', 'none');
		} else if ( v == '0' ) {
			jQuery('#is-shown-off').css('display', 'block');
			jQuery('#is-shown-on').css('display', 'none');
		}
	});
...
window.x_init_hook_team = function() {
	jQuery('#deal-buy-quantity-input').bind("keyup", function(){
		...
		var p = parseFloat(jQuery('#deal-team-price').attr('value'),10);
		p = isNaN(p) ? 0 : p; n = isNaN(n) ? 0 : n; var t = n * p; 
		t = X.team.number_format(t);
...
X.team.number_format = function(number, decimals, dec_point, thousands_sep) {
...
X.manage.loadtemplate_m = function(id) {
	window.location.href = WEB_ROOT + '/manage/system/template_m.php?id='+id;
};
