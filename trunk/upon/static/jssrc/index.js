...
jQuery.fn.checkForm = function (jform) {
    ...
    elements.blur(function (index) {
        return validator.check(jQuery(index))
    });
...
window.x_init_hook_click = function () {
    ...
    jQuery('a.ajaxlink').click(function () {
        ...
        if (link.indexOf('/delete') > 0 && !confirm('ȷ��ɾ������¼��')) {
            return false
...
window.x_init_hook_clock = function () {
    ...
    var e = function () {
        ...
        if (ls > 0) {
            ...
            var html = '<li><span>' + lh + '</span>Сʱ</li><li><span>' + lm + '</span>����</li><li><span>' + ls + '</span>��</li>';
...
X.misc.noticesms = function (tid, nid) {
    ...
    return X.get('/ajax/manage.php?action=noticesms&id=' + tid + '&nid=' + nid)
};
X.misc.needlogin = function () {
    return LOGINUID > 0 ? true : X.get('/ajax/system.php?action=needlogin')
};
...
///byli
X.link = {};
X.link.dialogquery = function () {
    var password = jQuery('#link-dialog-query').attr('value');
    if (id) return !X.get(WEB_ROOT + '/ajax/link.php?action=query&password=' + encodeURIComponent(password))
};
