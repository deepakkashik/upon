...
class Utility 
{
    ...
    static public function HumanTime($time=null, $forceDate=false)
    {
        ...
        if ( $forceDate || $interval > 30*86400 ){
            return strftime(INCLUDE_LIARBRY_UTILITY_DAYTIME,$time);
        }else if ( $interval > 86400 ){
            $number = intval($interval/86400);
            return $number . INCLUDE_LIARBRY_UTILITY_DAYS_AGO;
        }else if ( $interval > 3600 ){ // > 1 hour
            $number = intval($interval/3600);
            return $number . INCLUDE_LIARBRY_UTILITY_HOURS_AGO;
        }else if ( $interval >= 60 ){ // > 1 min
            $number = intval($interval/60);
            return $number . INCLUDE_LIARBRY_UTILITY_MININUTES_AGO;
        }else if ( 5 >= $interval){// < 5 second
            return $interval . INCLUDE_LIARBRY_UTILITY_JUST_NOW;
        }else{ // < 1 min
            return INCLUDE_LIARBRY_UTILITY_SECONDS_AGO;
        }
    }

    static public function GetDate() {
        $w = array(INCLUDE_LIARBRY_UTILITY_SUNDAY,
        		   INCLUDE_LIARBRY_UTILITY_MONDAY,
        		   INCLUDE_LIARBRY_UTILITY_TUESDAY,
        		   INCLUDE_LIARBRY_UTILITY_WENSDAY,
        		   INCLUDE_LIARBRY_UTILITY_THURSDAY,
        		   INCLUDE_LIARBRY_UTILITY_FRIDAY,
        		   INCLUDE_LIARBRY_UTILITY_SATURDAY);
        return $now = array(
                'date' => date(INCLUDE_LIARBRY_UTILITY_LONGDATE),
                'week' => INCLUDE_LIARBRY_UTILITY_WEEKDAY.$w[ date('w')]
                );
    }
	...
	// 郵便番号の判断
	static function IsZipcode($no) {
		$pattern = "/^(\d{7,7})$/"; 
		return preg_match($pattern, $no); 
	}
	//電話番号の判断
	...
	//文字の判断
	static function IsJpCharactor($no) {
		$pattern = "/^([一-龠]+[あ-ん])+$/"; 
		return preg_match($pattern, $no); 
	}
	//　最少と最大の長さを判断する
	...
	//　エラーメッセージの文字列を結合
	static function AddStr($var, $varSet) {
		if ($var) {
			return $var."<br/>"."・".$varSet;
		}
		return "・".$varSet;
	}
}
