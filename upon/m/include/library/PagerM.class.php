...
class PagerM{
	...
	public function GenBasic() {
		$r = $this->paginate();
		$buffer = null;
		$index = INCLUDE_LIARBRY_PAGER_FIRSTPAGE;
		$pre = INCLUDE_LIARBRY_PAGER_LASTPAGE;
		$next = INCLUDE_LIARBRY_PAGER_NEXTPAGE;
		...
		$buffer .= '';
		if ($this->pageNo > 1) {
			$buffer .= "<a href='".$this->genURL($this->pageString,1)."'>{$index}</a>  <a href='".$this->genURL($this->pageString,$this->pageNo-1)."'>{$pre}</a>";
		}
