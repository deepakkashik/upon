...
class DB
{
	...
	static public function LimitQuery($table, $options=array(),$conditionconnect='=')
	{
		...
		if($conditionconnect=='L'){
			$condition = self::BuildConditionLike( $condition );
		}else{
	...
	static public function BuildConditionLike($condition=array(), $logic='AND')
	{
		if ( is_string( $condition ) || is_null($condition) )
			return $condition;
	...
	static public function InsertToDB( $table, $condition )
	{
		self::Instance();
